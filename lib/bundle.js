...
var simulation = d3.forceSimulation().force("charge", d3.forceManyBody().strength(-650)).force("link", d3.forceLink().distance(110)).force("x", d3.forceX(width / 2)).force("y", d3.forceY(height / 2)).force("collide", d3.forceCollide(25)).on("tick", ticked);
...
function render() {
  ...
  d3.json(data, function (data) {
    ...
    var circle = d3.selectAll(".parent").append("circle").attr("r", 60).attr("opacity", 0.4).on("dblclick", function (d) {
      return toggleZoom(d);
    }).on("mouseenter", function () {
      d3.select(this).transition().duration(1000).attr("r", 120);
    }).on("mouseleave", function (d) {
      d3.select(this).transition().duration(1000).attr("r", 60);
    });
