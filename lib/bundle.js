...
function render(data) {
  ...
  var img = node.filter(".leaf").append("image").attr("xlink:href", function (d) {
    ...
    d3.select(this).transition().duration(1000).attr("height", 150).attr("width", 150).attr("opacity", 1).on("end", function (d) {
      return tip(d);
    });
  ...
  var circle = d3.selectAll(".parent").append("circle").attr("r", 80).attr("opacity", 0.4).on("mouseenter", function () {
    ...
    d3.select(this.parentElement.children[0]).transition().duration(1000).attr("font-size", "50px").attr("font-weight", "700");
    // .attr("transform", d => "translate(" + -100 + "," + -100 + ")");
  }).on("mouseleave", function (d) {
    d3.select(this.parentElement.children[0]).transition().duration(1000).attr("font-size", "15px").attr("font-weight", "100");
    // .attr("transform", d => "translate(" + 35 + "," + 35 + ")");
